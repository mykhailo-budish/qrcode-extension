...
fs.readdirSync(path.join(__dirname, inputDir, 'icons')).forEach(file => files.push(`icons/${file}`));
...
while (mainContentWithCode.match(requireRegex) !== null) {
  mainContentWithCode = mainContentWithCode.replace(requireRegex, (match, p1) => {
    const filePath = path.join(__dirname, inputDir, p1);
    return fs.readFileSync(filePath, 'utf8');
  });
  hasImports = mainContentWithCode.match(requireRegex) !== null;
}
